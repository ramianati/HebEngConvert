...
class SettingsDialog(ctk.CTkToplevel):
    def __init__(self, parent, current_hotkey, current_port, on_save_callback):
        ...
        ctk.CTkLabel(self.info_frame, text="1.0.2", font=info_font).grid(row=1, column=1, sticky="w", padx=10)
        ...
        # Clean display: remove <> from the entry field
        ...
        ctk.CTkLabel(self.port_frame, text="Instance Port:", font=label_font).grid(row=0, column=0, sticky="w")
    ...
    def _on_key_press(self, key):
        ...
        k_str = ""
        # Check for alphanumeric via vk (ensures 'a' instead of ascii codes when ctrl is held)
        if hasattr(key, 'vk') and 65 <= key.vk <= 90:
            ...
            if name in ['ctrl', 'alt', 'shift', 'cmd', 'win']:
                k_str = name # No <> here, we add it back for main.py mapping during save
            else:
                k_str = name
        else:
        ...
        if k_str:
            ...
            # Display: Sort modifiers (e.g. ctrl, alt) then others
    ...
    def _on_key_release(self, key):
        if not self.is_capturing: return
        # Simple heuristic: stop if a non-modifier was released
    ...
    def _save(self):
        ...
        if new_hotkey_raw and new_port:
            try:
                ...
                # Re-add <> for pynput mapping internal storage
                ...
                self.on_save_callback(save_hk, port_int)
                self.destroy()
            except ValueError:
                self.status_label.configure(text="Error: Port must be a number", text_color="red")

class ClipboardWindow(ctk.CTk):
    def __init__(self, on_refresh_callback=None, current_hotkey="ctrl+alt+d", current_port=54321, on_settings_save=None):
        super().__init__()
    ...
    def open_settings(self):
        SettingsDialog(self, self.current_hotkey, self.current_port, self._on_settings_applied)
